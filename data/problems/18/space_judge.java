/*
Judge to 'space' by Børge Nordli, for IDI Open 2013.
*/

import java.io.*;
import java.util.*;

public class space_judge {
  ...
  static ArrayList<Double> expected = new ArrayList<Double>();
  ...
  public static void main(String[] a) {
    int T = INT();
    for (int i = 0; i < T; ++i) {
      expected.add(new space_judge().go());
    }
...
//      System.out.println("  Answered " + answer);
      double absError = Math.abs(expected.get(i) - answer);
      if (!(absError < EPS || absError / answer < EPS)) {
